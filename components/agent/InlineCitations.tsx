...
export function InlineCitations({ sources }: InlineCitationsProps) {
  ...
  return (
    <div className="border-t-2 border-black pt-2.5 mt-2 space-y-1.5 select-none">
      <div className="font-mono text-[10px] uppercase tracking-widest font-bold text-neutral-600 pb-1">
      ...
      <div className="space-y-1">
        {sources.map((src, i) => (
          ...
          >
            <span className="font-mono text-[10px] font-bold text-black bg-neutral-100 border border-black px-1 py-0.5 shrink-0">
              [{i + 1}]
            </span>
            <span className="truncate min-w-0 flex-1">{src.title || src.url}</span>
            <span className="font-mono text-[10px] text-neutral-600 group-hover:text-black shrink-0 ml-auto border border-neutral-200 px-1 py-0.5 flex items-center gap-1">
