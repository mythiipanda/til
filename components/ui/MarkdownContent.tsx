    ...
    <div className={`font-body text-inherit leading-relaxed ${className}`}>
      <ReactMarkdown
        components={{
          ...
          blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-current pl-3 my-2 italic font-serif text-inherit/90">
              {children}
            </blockquote>
          ),
          code: ({ children }) => (
            <code className="font-mono text-[11px] bg-neutral-200/50 text-inherit px-1 py-0.5 border border-current/20">
              {children}
            </code>
          ),
          a: ({ href, children }) => (
            <a
              ...
              className="underline decoration-1 underline-offset-2 hover:opacity-75 font-mono text-[11px]"
            >
              {children}
            </a>
          ),
